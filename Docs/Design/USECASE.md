...
- **영수증 정보 추출(OCR)**: "세금 영수증 지출분석" 한 항목을 등록 → OCR 추출 → 지출 분류 → 경비처리 가능성 분석 네 단계로 구체화했다. 한때 네 단계 전체를 추가 기능(추후 개발)으로 돌렸으나, 지출관리 화면이 붙으면서 다시 메인 범위에 넣었다.
- **사업계획서 초안 생성·AI 예비진단·아이디어 어시스턴트**: 지원사업 신청(로드맵 C단계)에 사업계획서가 필요하지만 작성 도구가 없어 추가했다. 초안 생성과 채점, 작성 도움 대화를 서로 다른 행위로 보아 세 유스케이스로 나눴다.
