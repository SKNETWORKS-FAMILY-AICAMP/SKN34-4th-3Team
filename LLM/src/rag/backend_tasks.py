...
# 카드 이용내역 캡처에는 "이용금액 29,998원" 아래에 "결제금액 0원"(해외 수수료·환산용 칸)이
# 따로 붙어 있어, "결제금액 줄을 기준으로" 하라는 지시만으로는 총액을 0원으로 뽑았다(2026-09-24).
_RECEIPT_AMOUNT_RULES = (
    "amount(총액)는 이 거래에서 실제로 쓴 돈입니다. 합계·결제금액·청구액·이용금액·승인금액처럼 "
    "최종 금액이 적힌 줄을 기준으로 하되, 이런 줄이 여러 개면 0보다 큰 금액을 고르세요. "
    "카드 이용내역 화면에는 '이용금액 29,998원' 아래에 '결제금액 0원', '해외서비스수수료 0원'처럼 "
    "수수료·환산용 0원 줄이 따로 붙는 경우가 있으니, 0원 줄을 총액으로 쓰지 마세요. "
    "할인이 있으면 할인 전 합계가 아니라 할인 후 실제로 결제한 금액(결제금액·청구액)을 쓰세요. "
    "0보다 큰 금액이 하나도 확인되지 않을 때만 null로 두세요."
)
