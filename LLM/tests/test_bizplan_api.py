...
from src.features.document_processing import PdfTextNotFoundError
...
def test_business_plan_returns_generated_sections(tmp_path: Path) -> None:
    model = FakeStructuredChatModel(
        {BusinessPlanGeneration: {"sections": PSST_SECTIONS, "summary": "한 줄 요약"}}
    )
    client = build_client(tmp_path / "index.json", llm_factory=lambda: model)

    response = client.post(
        "/rag/business-plan",
        json={"businessName": "동네 반찬 구독", "problem": "퇴근 후 요리할 시간이 없다"},
    )

    assert response.status_code == 200
    body = response.json()
    assert [s["key"] for s in body["sections"]] == ["problem", "solution", "scaleUp", "team"]
    assert body["summary"] == "한 줄 요약"
    assert body["llmUsed"] is True
    assert "동네 반찬 구독" in model.last_prompt_text
...
def test_business_plan_rejects_oversized_template(tmp_path: Path) -> None:
    client = build_client(tmp_path / "index.json")

    response = client.post("/rag/business-plan", json={"templateText": "가" * 6001})
...
def test_template_file_extracts_pdf_text(
    tmp_path: Path, monkeypatch: pytest.MonkeyPatch
) -> None:
    monkeypatch.setattr(rag_routes, "extract_pdf_text", lambda _data: "1. 창업아이템 개요")
    client = build_client(tmp_path / "index.json")

    response = client.post(
        "/rag/business-plan-template-file",
        files={"file": ("양식.pdf", b"%PDF-1.4 fake", "application/pdf")},
    )

    assert response.status_code == 200
    assert response.json() == {"templateText": "1. 창업아이템 개요"}


def test_template_file_truncates_long_text(
    tmp_path: Path, monkeypatch: pytest.MonkeyPatch
) -> None:
    monkeypatch.setattr(rag_routes, "extract_pdf_text", lambda _data: "가" * 10000)
    client = build_client(tmp_path / "index.json")
    ...
    assert response.status_code == 200
    # 초안 생성 요청의 templateText 제한(6000자)에 맞춰 잘라 보낸다.
    assert len(response.json()["templateText"]) == rag_routes.MAX_TEMPLATE_TEXT_CHARACTERS


def test_template_file_rejects_non_pdf(tmp_path: Path) -> None:
    client = build_client(tmp_path / "index.json")

    response = client.post(
        "/rag/business-plan-template-file",
        files={"file": ("양식.hwp", b"HWP", "application/x-hwp")},
    )

    assert response.status_code == 415


def test_template_file_without_text_is_422(
    tmp_path: Path, monkeypatch: pytest.MonkeyPatch
) -> None:
    def no_text(_data: bytes) -> str:
        raise PdfTextNotFoundError("No extractable text in PDF")

    monkeypatch.setattr(rag_routes, "extract_pdf_text", no_text)
    client = build_client(tmp_path / "index.json")

    response = client.post(
        "/rag/business-plan-template-file",
        files={"file": ("스캔본.pdf", b"%PDF-1.4 fake", "application/pdf")},
    )
